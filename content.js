...
chrome.storage.local.get(
    ...
    (result) => {
        InvestingDecimalPointSetting = result.investingDecimalSetting !== undefined ? parseInt(result.investingDecimalSetting) : 1; // Default to 1 if not set
        ZiraatDecimalPointSetting = result.ziraatDecimalSetting !== undefined ? parseInt(result.ziraatDecimalSetting) : 0; // Default to 0 if not set
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.action === "parse-Ziraat") {
        let data = parseZiraatPortfolioData();
        chrome.runtime.sendMessage(
            { action: "saveData", data: data, source: "Ziraat", id: message.id },
            (response) => {
    ...
    } else if (message.action === "parse-Investing") {
        let data = parseInvestingPortfolioData();
        chrome.runtime.sendMessage(
            { action: "saveData", data: data, source: "Investing", id: message.id },
            (response) => {
                if (chrome.runtime.lastError) {
                    console.error("Error sending message to background script:", chrome.runtime.lastError.message);
                } else {
